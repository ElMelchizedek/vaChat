...
exports.handler = async (event) => {
	...
	try {
		...
		const paramResponse = await clientSSM.send(command);
	} catch (error) {
	...
	try {
		const input = {
			TargetArn: paramResponse,
			Message: messageContent,
			MessageAttributes: {
				"account": {
					DataType: "Number",
					StringValue: toString(messageAccount),
				},
				"timestamp": {
					DataType: "Number",
					StringValue: toString(messageTime),
				}
			},
		}
		...
		console.error("Error executing PublishCommand whilst trying to push parsed stream record to channel endpoint topic: ", record);
	}
}